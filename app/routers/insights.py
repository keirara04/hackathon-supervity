...
async def get_insights(limit: int = 500):
    try:
        ...
        runs = await sb_get(
            "run_log",
            {"select": "path,mttr_minutes,sla_state_before,department,category", "order": "entered_at.desc", "limit": str(limit)},
        )
    ...
    return {"insights": insights, "generated_from": {"policy_evaluations": total_evals, "run_log_rows": len(runs)}}
